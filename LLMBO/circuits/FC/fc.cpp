...
using namespace std;
const int f_num = 18;
const string cir = "fc_";

int sim_gen(const string& id, double* s){
    string cmd = "echo '* Sim\n.inc ./param" + id + ".inc' > " + cir + id + ".sp && cat fc.sp >> " + cir + id + ".sp";
    system(cmd.c_str());

    string file = "param" + id + ".inc";
    ofstream fout(file.c_str(), ios_base::out);
    fout << ".param r1 =  " << s[0] << endl;
    fout << ".param r2 =  " << s[1] << endl;
    fout << ".param l1 =  " << s[2] << endl;
    fout << ".param l3 =  " << s[3] << endl;
    fout << ".param l4 =  " << s[4] << endl;
    fout << ".param l6 =  " << s[5] << endl;
    fout << ".param l8 =  " << s[6] << endl;
    fout << ".param l9 =  " << s[7] << endl;
    fout << ".param l10 = " << s[8] << endl;
    fout << ".param l13 = " << s[9] << endl;
    fout << ".param w15 = " << s[10] << endl;
    fout << ".param w1 =  " << s[11] << endl;
    fout << ".param w4 =  " << s[12] << endl;
    fout << ".param w6 =  " << s[13] << endl;
    fout << ".param w8 =  " << s[14] << endl;
    fout << ".param w9 =  " << s[15] << endl;
    fout << ".param w10 = " << s[16] << endl;
    fout << ".param w13 = " << s[17] << endl;
    fout.close();
    return 0;
}

int read_vol(const string& id, vector<double>& vol) {
    string filename = cir + id + ".ic1";
    ...
    while(getline(fin, line)) {
        ...
        if(node == "3") {vol[0] = v; ++cnt;}
        else if(node == "4") {vol[1] = v; ++cnt;}
        else if(node == "5") {vol[2] = v; ++cnt;}
        else if(node == "6") {vol[3] = v; ++cnt;}
        else if(node == "7") {vol[4] = v; ++cnt;}
        else if(node == "8") {vol[5] = v; ++cnt;}
        else if(node == "9") {vol[6] = v; ++cnt;}
        else if(node == "out") {vol[7] = v; ++cnt;}
        else if(node == "11") {vol[8] = v; ++cnt;}
        else if(node == "12") {vol[9] = v; ++cnt;}
        else if(node == "13") {vol[10] = v; ++cnt;}
        else if(node == "14") {vol[11] = v; ++cnt;}
        if(cnt == 12) break;
    }
    fin.close();
    return 1;
