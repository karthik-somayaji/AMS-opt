...
def normalize_fvals(f_vals: dict, ckt: dict):
    ...
    # normalize the objective function values
    f_vals['gbw_normed'] = (np.log(gbw) - np.log(gbw_range[0])) / (np.log(gbw_range[1]) - np.log(gbw_range[0]))#(gbw - gbw_range[0]) / (gbw_range[1] - gbw_range[0])
    ...
    f_vals['pow_normed'] = (np.log(pow) - np.log(pow_range[0])) / ( np.log(pow_range[1]) - np.log(pow_range[0]))
    ...
    # f_vals['pow_normed'] = min(1, max(0, f_vals['pow_normed']))
    if f_vals['pow_normed'] <=0:
        f_vals['pow_normed'] = 1
    else:
        f_vals['pow_normed'] = -1*f_vals['pow_normed']
...
def objective(f_vals: dict, ckt: dict):
    ...
    # merits
    merits = ['gbw', 'gain', 'cmrr', 'pow']#, 'pm']#, 'pow']
    fom = 0
    for merit in merits:
        # if merit in ['pm', 'pow']:
        ...
        #else:
        fom += ckt[f'{merit}_weight'] * f_vals[f'{merit}_normed']
        # print(merit, ckt[f'{merit}_weight'], f_vals[f'{merit}_normed'] , ckt[f'{merit}_weight'] * f_vals[f'{merit}_normed'], fom)
