...
from backend.spice_ldo.objective import objective, read_results # noqa: E402
#from backend.spice_comp.aux_info import read_work_region # noqa: E402


def hspice_eval_f_ldo(point_to_evaluate, args):
    '''
    ...
    with open(f'{ckt_dir}/{ckt_name}.mt0') as f:
        lines = f.readlines()
        q_curr = read_results(lines[4].split()[0], 'q_curr', args)
        stability = float(lines[5].split()[1]) - float(lines[5].split()[0])#read_results(lines[4].split()[1], 'ugf', args)
        output_voltage_difference = 1.2 - float(lines[4].split()[3])
    
    # update fvals
    fvals['q_curr'] = np.abs(q_curr)
