...
from backend.spice_comp.objective import objective, read_results # noqa: E402
from backend.spice_comp.aux_info import read_work_region # noqa: E402


def hspice_eval_f_comp(point_to_evaluate, args):
    '''
    ...
    with open(f'{ckt_dir}/{ckt_name}.ma0') as f:
        ...
        ugf = read_results(lines[4].split()[1], 'ugf', args)
    with open(f'{ckt_dir}/{ckt_name}.mt0') as f:
        lines = f.readlines()
        hyst_err = read_results(lines[4].split()[2], 'hyst_err', args)
        offset = read_results(lines[4].split()[3], 'offset', args)
    # read results from {ckt_name}.dp0
    with open(f'{ckt_dir}/{ckt_name}.dp0') as f:
        lines = f.readlines()
        #pow_vol = read_results(lines[31].split()[1], 'pow_vol', args)
        #pow_cur = read_results(lines[55].split()[1], 'pow_cur', args)
        #pow = pow_vol + pow_cur
        work_regions = read_work_region(lines)
    # update fvals
    fvals['ugf'] = ugf
    fvals['gain'] = gain
    fvals['hyst_err'] = hyst_err
    fvals['offset'] = offset
